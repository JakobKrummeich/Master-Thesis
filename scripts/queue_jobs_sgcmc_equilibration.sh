...
for N in {500,1000,2000,4000,8000,16000}; do
	for temperature in {0.7468,0.7469,0.7470,0.7471,0.7472}; do
		for run_offset in {0,20,40,60}; do
			...
			srun_command="srun --ntasks=1 --error=error_stream_output/N=${N}_%J.err ./sgcmc_equilibrate_${N} ${temperature} "/home1/krummeich/Master-Thesis/data/semi_gcmc_no_field/sorted_data/finite_size_scaling_roh=0.75_long_runs/N=${N}/T=0.750000/States_N=${N}_T=0.750000_AvgDens=0.750000_MCRuns=10000000_epsAB=0.100000.dat" ${states_to_skip_per_run} ${run_offset} &
