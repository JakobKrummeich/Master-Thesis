...
int main(int argc, char* argv[]){
	...
	#pragma omp parallel num_threads(NUMBER_OF_THREADS)
	{
		...
		for (int RunCount = 0; RunCount < NUMBER_OF_RUNS; RunCount++){
			...
			S.setFileNameString(RunCount+RunNumberOffset, MCModus::SGCMC);
			S.resetCountersAndBuffers();
			S.runSGCMCSimulationForSingleTemperatureTimeControlled(RunCount+RunNumberOffset, MAX_RUNTIME_IN_MINUTES);
		}
	}
}
